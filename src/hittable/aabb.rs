...
impl AABB {
    ...
    ///
    pub fn new(min: Point3, max: Poing3) -> Self {
        Self {
            min: min,
            max: max,
        }
...
impl Hittable for AABB {
    ...
    fn is_hit(&self, ray: Ray, min_dist: f64, max_dist: f64) -> bool {
        ...
        let x_min_dist = (self.min.x - ray.origin.x) * inverse_x_direction;
        let x_max_dist = (self.max.x - ray.origin.x) * inverse_x_direction;
        if (inverse_x_direction < 0.0) { swap(x_min_dist, x_max_dist) }

        let inverse_y_direction = ray.direction.y;
        let y_min_dist = (self.min.y - ray.origin.y) * inverse_y_direction;
        let y_max_dist = (self.max.y - ray.origin.y) * inverse_y_direction;
        if (inverse_y_direction < 0.0) { swap(y_min_dist, y_max_dist) }

        let inverse_z_direction = ray.direction.z;
        let z_min_dist = (self.min.z - ray.origin.z) * inverse_z_direction;
        let z_max_dist = (self.max.z - ray.origin.z) * inverse_z_direction;
        if (inverse_z_direction < 0.0) { swap(z_min_dist, z_max_dist) }
